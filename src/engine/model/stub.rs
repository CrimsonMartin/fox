...
// ---------------------------------------------------------------------------
// GemmaThinkingStubModel — simulates Gemma 4 channel-based thinking
// ---------------------------------------------------------------------------
//
// Token sequence:
//   prefill  → 400  "<|channel>"
//   decode 0 → 401  "thought"
//   decode 1 → 402  "<channel|>"
//   decode 2 → 403  "answer"
//   decode 3+ → 2   (EOS)
//
// Gemma does NOT use supports_thinking() — the output filter detects
// <|channel>…<channel|> blocks independently of the <think> mechanism.
pub struct GemmaThinkingStubModel {
    decode_step: AtomicUsize,
}

impl GemmaThinkingStubModel {
    pub fn new() -> Self {
        Self {
            decode_step: AtomicUsize::new(0),
        }
...
impl Model for GemmaThinkingStubModel {
    fn prefill_sync(
        &self,
        req_ids: &[u64],
        requests: &[InferenceRequestForModel],
    ) -> Result<Vec<(u64, Logits, usize)>> {
        self.decode_step.store(0, Ordering::SeqCst);
        Ok(req_ids
            .iter()
            .zip(requests.iter())
            .map(|(&id, r)| (id, Logits::new(vec![], 400), r.prompt_tokens.len()))
            .collect())
    }
    ...
    fn decode_sync(
        ...
        const DECODE_SEQ: &[i32] = &[401, 402, 403, 2];
        let step = self.decode_step.fetch_add(1, Ordering::SeqCst);
        let token = DECODE_SEQ.get(step).copied().unwrap_or(2);
        Ok(req_ids
            .iter()
    ...
    fn token_to_piece(&self, token: i32) -> Result<String> {
        Ok(match token {
            2 => String::new(),
            400 => "<|channel>".to_string(),
            401 => "thought".to_string(),
            402 => "<channel|>".to_string(),
            403 => "answer".to_string(),
            _ => "hi ".to_string(),
        })
    }
