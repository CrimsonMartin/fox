...
fn build_oom_fallbacks(original_ctx: Option<u32>, type_k: u32, type_v: u32) -> Vec<OomFallback> {
    ...
    // Halve context repeatedly, keeping the user's KV types.
    // For auto-detect (None), start from 65536 since we don't know the model's trained context.
    ...
    // If not already using q8_0, try small contexts with quantized KV cache.
...
pub(super) async fn load_model(
    ...
    let (model, degraded, effective_type_k, effective_type_v) = {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // Progressively halve context, then try quantized KV.
